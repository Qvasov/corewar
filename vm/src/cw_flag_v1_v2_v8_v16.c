...
{
	ft_bprintf(&data->fstr, "Process %llu hasn't lived for %d cycles (CTD %lld)\n",
...
void	flag_v16(t_data *data, t_cur *cursor)
{
	...
	i = -1;
	if (data->o_flag)
		ft_bprintf(&data->fstr, "ADV %d (0x%.4x -> 0x%.4x) ", cursor->byte_to_next_op,
	...
	while (++i < cursor->byte_to_next_op)
		ft_bprintf(&data->fstr, "%.2hhx ", data->vm.arena[(cursor->pc + i) % MEM_SIZE]);

...
int		get_ind_value(t_int arg, t_cur *cursor, t_data *data)
{
	...
	size = (cursor->op_code == LLD && data->o_flag) ? 1 : 3;
	i = -1;
	while (++i <= size)
		arg.byte[size - i] = data->vm.arena[(addr + i) % MEM_SIZE];
	if (size == 1) 							//check отрицательные
...
int		get_arg(uint8_t arg_code, int8_t arg_pos, uint8_t *arena, t_cur *cursor)
{
	...
	{
		i = -1;
		j = (arg_code == DIR_CODE && !g_op[cursor->op_code].dir_size) ? 3 : 1;
		while (++i <= j)
		...
		t_data *data)
{
	...
	args_size = 2;
	i = -1;
	if (++i < g_op[cursor->op_code].args_count)
	{
		arg[i].num = get_arg(args_code.a.arg1, args_size, data->vm.arena, cursor);
		args_size += data->size[args_code.a.arg1];
	}
	if (++i < g_op[cursor->op_code].args_count)
	{
		arg[i].num = get_arg(args_code.a.arg2, args_size, data->vm.arena, cursor);
		args_size += data->size[args_code.a.arg2];
	}
	if (++i < g_op[cursor->op_code].args_count)
	{
		arg[i].num = get_arg(args_code.a.arg3, args_size, data->vm.arena, cursor);

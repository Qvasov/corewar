		...
		const int8_t *size)
{
	...
	if (1 <= g_op[cursor->op_code].args_count && args_code.a.arg1 == REG_CODE)
	{
		reg_pos = (cursor->pc + 2) % MEM_SIZE;
		if (!(vm->arena[reg_pos] >= 1 && vm->arena[reg_pos] <= 16))
			return (1);
	}
	if (2 <= g_op[cursor->op_code].args_count && args_code.a.arg2 == REG_CODE)
	{
		reg_pos = (cursor->pc + 2 + size[args_code.a.arg1]) % MEM_SIZE;
		if (!(vm->arena[reg_pos] >= 1 && vm->arena[reg_pos] <= 16))
			return (1);
	}
	if (3 <= g_op[cursor->op_code].args_count && args_code.a.arg3 == REG_CODE)
	{
...
int			ft_valid_op_code_and_reg(t_data *data, t_cur *cursor)
{
	...
	if (g_op[cursor->op_code].args_type_code)
	{
		...
		if (data->valid[args_code.a.arg1](0, cursor->op_code) ||
			data->valid[args_code.a.arg2](1, cursor->op_code) ||
			data->valid[args_code.a.arg3](2, cursor->op_code) ||
			data->valid[args_code.a.arg4](3, cursor->op_code) ||

...
void		web_cycle(t_data *data)
{
	...
	while (++n < data->vm.num_of_players)
	{
		...
		while (++i < 17)
		{
			if (data->web.curs[n][i] > 0)
			{
				...
				ft_fprintf(data->web.fd, "%d, %d, %d", data->web.curs[n][i] - 1, i, n + 1);
				data->web.f = 1;
			}
	...
	while(++i < MEM_SIZE)
	{
		...
		{
			data->web.f ? ft_fprintf(data->web.fd, ", ", i, data->web.change[i], data->vm.cycles_to_die) : 1;
			data->web.arena[i] = data->web.change[i]; /*change(data->web.arena[i], data->web.change[i]);*/
			data->web.change[i] = -1;
...
//	ft_bzero(data->web.change, 4 * MEM_SIZE);
}
